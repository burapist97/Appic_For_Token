...
class TestOtomasyonApp(ctk.CTk):
    def __init__(self):
        ...
        self.adb_yolu = os.path.join(self.ana_dizin, "platform-tools", "adb.exe") if os.name == 'nt' else "adb"
    ...
    def ekran_yayini_dongusu(self):
        ...
        while self.kayit_aktif:
            try:
                islem = subprocess.run([self.adb_yolu, "exec-out", "screencap", "-p"], capture_output=True, creationflags=c_flags)
                if islem.stdout:
                    img = Image.open(io.BytesIO(islem.stdout))
                    ...
                    self.lbl_ekran.configure(image=ctk_img, text="")
            except Exception: pass
            time.sleep(0.3)
